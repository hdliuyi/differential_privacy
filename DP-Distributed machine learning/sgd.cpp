// sgd.cpp : Defines the entry point for the console application.
//
...
/*
Each iteration asks a group of users to submit their gradients//本地化思想
*/
double MGD(string task, string method, vec& beta, vector<int> &train, vector<int>& test, mat &X, rowvec &Y_linear, rowvec &Y_logistic, float eps) {
	...
	for (int i = 0; i < folds; i = i + 1) {//folds - nftest denotes the number the first 80% batch.
		...
		for (int id = 0; id < bs; id++) {//each user in one group
			...
			for (int ki = 0; ki < k; ki++) {//choose k attributes to publish  属性k
				uniform_int_distribution<int> distribution(0, currentSampleDomain.size() - 1);
				int idxid = distribution(generator);//分布式
				...
				noisy = noisy + (double)d / k*pai;
			}
			double noisy_add = (noisy);
			fstream no;
			no.open("noisy.txt", ios::app);
			no << noisy_add << endl;
			no.close();

			fstream go;
			go.open("gradone.txt", ios::app);
			go << gradone[0] << endl;
			go.close();
		}

		fstream sg;
		sg.open("sgrad.txt", ios::app);
		sg << sgrad[0] << endl;
		sg.close();
